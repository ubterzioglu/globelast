...
export async function POST(request: Request) {
  ...
  const { count: userPinCount, error: userPinCountError } = await supabase
    .from('event_pins')
    .select('id', { count: 'exact', head: true })
    .eq('user_id', userId)
    .eq('event_key', EVENT_KEY);

  if (userPinCountError) {
    return NextResponse.json({ error: userPinCountError.message }, { status: 500 });
  }
  ...
  if ((userPinCount ?? 0) >= MAX_PINS_PER_USER) {
    return NextResponse.json(
      { error: `Bu etkinlik için en fazla ${MAX_PINS_PER_USER} pin gönderebilirsin.` },
      { status: 409 }
    );
  }
